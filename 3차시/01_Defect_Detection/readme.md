...
3. 학습 완료 후 모델 경로 복사 (예: `runs/defect/20260110_123456/train/weights/best.pt`)
4. 학습 시간: 약 30분~1시간 (GPU 성능에 따라 다름)

#### 4-2. 02_test.ipynb (모델 평가)
1. Google Drive에서 `code/02_test.ipynb` 열기
2. `MODEL_PATH` 변수를 **4-1에서 복사한 경로**로 수정
   ```python
   MODEL_PATH = '/content/drive/MyDrive/.../runs/defect/20260110_123456/train/weights/best.pt'
   ```
3. 셀을 순서대로 실행
...
└── runs/
    └── defect/
        └── 20260110_123456/  (실행 시간에 따라 폴더명 다름)
            ├── train/
            │   ├── weights/
            │   │   └── best.pt  (학습된 모델)
            │   ├── results.png   (학습 그래프)
